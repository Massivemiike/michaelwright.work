...
- **The Plan 2B final-review fix wave governs where the code now differs from this document's blocks** (commits `6b8ee0a`, `4552804`, `b903a3d`, `17a5f77`, `2b690d7` and `177b1ad`; no pin moves): the host's `start` error path and its seed and log checks, the client's robustness, and the sweep's knob validation and merge completeness, plus the tests it adds and the comments it narrows to what the code guarantees. After it, the Arcfire suite is 277 tests (+ 1 skipped) and the whole suite 530 (+ 11 skipped); the worker bundle is 40.2 KiB.
