...
- **Entry points.** `resolveTurnPoints(m, input)` returns the same state and points with no Timeline. `resolveWeapon(m, def, input, record)` fires any definition without a roster entry: 2B's cheap probe shell (in record mode its `blast` or `out` event gives the landing point) and Plan 3's draft preview can both use it. *Done in 2B, differently:* only the AI's search uses `resolveTurnPoints`; the probe flies a shell weapon's probe model with the sim's own `muzzle` / `launchShell` / `stepShell` directly (a beam's is its line on the beam dial), never a `resolveWeapon` shell; the draft preview is the worker's `preview` request, `resolveTurn` on a fixed flat board; and verification replays every turn, the AI's and the human's, through `applyTurn` (so `resolveTurn`). See spec §9.1 (Entry points) and §3.4.
